...
function dTree(objName){
    ...
    this.icon = { /* 指定各个图标 */
        root: projectName + '/resources/dtree/img/base.gif', /* 根节点图标 */
        folder: projectName + '/resources/dtree/img/folder.gif', /* 未展开节点图标 */
        folderOpen: projectName + '/resources/dtree/img/folderopen.gif', /* 展开后节点图标 */
        node: projectName + '/resources/dtree/img/page.gif', /* 叶子节点(无节点的节点)图标 */
        empty: projectName + '/resources/dtree/img/empty.gif', /* 空白图标 */
        line: projectName + '/resources/dtree/img/line.gif', /* 竖向连接线图标 */
        join: projectName + '/resources/dtree/img/join.gif', /* 兼具水平、竖向连接线的图标 */
        joinBottom: projectName + '/resources/dtree/img/joinbottom.gif', /* 底端连接线图标 */
        plus: projectName + '/resources/dtree/img/plus.gif', /* 带连接线的+图标 */
        plusBottom: projectName + '/resources/dtree/img/plusbottom.gif', /* 带连接线的底端+图标 */
        minus: projectName + '/resources/dtree/img/minus.gif', /* 带连接线的-图标 */
        minusBottom: projectName + '/resources/dtree/img/minusbottom.gif', /* 带连接线的底端-图标 */
        nlPlus: projectName + '/resources/dtree/img/nolines_plus.gif', /* 不带连接线的+图标 */
        nlMinus: projectName + '/resources/dtree/img/nolines_minus.gif' /* 不带连接线的-图标 */
    };
    this.obj = objName;
    this.aNodes = [];
    this.aIndent = [];
...
dTree.prototype.node = function(node, nodeId){
    ...
    //add href for nodes use this.url 
	/*
    if (node.url) {
	...
	if (node.url) {
        ...
        if (this.config.useStatusText) 
            str += ' onmouseover="window.status=\'' + node.name + '\';return true;" onmouseout="window.status=\'\';return true;" ';
        str += ' onclick="javascript: ' + this.obj + '.getChildren(' + node.id + ', setTable' + ')"';
        str += '>';
    }
...
/**
	 * @brief find the node whoes id equals id and get all children node of this node.
	 * if the node has no child, request /servlet/FileShowServlet?orgID=id to get file list or file content from database or HIVE. orgID may be id of node or file name
	 * @param id:node id or file name; funName: function name which handle the response string.
	 * @return 
	 * @exception
	 * @version 0.1
	 * @author mathyrs
...
dTree.prototype.getChildren = function(id, funName){
	var node;
	for (var n = 0; n < this.aNodes.length; n++) {
    ...
    if (!node || !node._hc) {
        ...
        if (xhr) {
            ...
            var projectName = pathName.substring(0, pathName.substr(1).indexOf('/') + 1);
            	...
            	xhr.open("GET", localhostPath + projectName + "/servlet/FileShowServlet?orgID=" + id, true);
            	xhr.send(null);
        	}
        }
    else {
		...
		for (var n = 0; n < this.aNodes.length; n++) {
			...
			if (this.aNodes[n].pid == node.id) {
				str = str + this.aNodes[n].name + ";" + this.aNodes[n].id + "<br/>";
			}
		}
